...
contract FeeCollector {
    ...
    function convert2(address token) public {
        // At least we try to make front-running harder to do.
        require(msg.sender == tx.origin, "do not convert from contract");
        uint256 amount = IERC20(token).balanceOf(address(this));
        uint256 wethAmount = _toWETH(token, amount);
        _to_PPAY(wethAmount);
    }
    ...
    function _safeTransferCommissions(address token, uint256 amountIn)
        internal
    {
        _safeTransfer(token, staking, amountIn.mul(stakingPercents) / 100);
        _safeTransfer(token, governance, amountIn.mul(governancePercents) / 100);
        _safeTransfer(token, operations, amountIn.mul(operationsPercents) / 100);
    }
